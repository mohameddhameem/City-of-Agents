  ...
  for (int len = 2; len < n; ++len) {
    for (int i = 0; i + len <= n; ++i) {
      ...
      if (i) {
        ...
        for (int ind : colors[lc])
          if (i <= ind && ind < j) dp[i][j][0] = min(dp[i][j][0], dp[i][ind - 1][0] + dp[ind + 1][j][0]);
      }
      if (j < n - 1) {
        int rc = arr[j + 1];
        dp[i][j][1] = min(dp[i + 1][j][0] + (arr[i] != rc), dp[i][j - 1][1] + (arr[j] != rc));
        for (int ind : colors[rc])
          if (i <= ind && ind < j) dp[i][j][1] = min(dp[i][j][1], dp[i][ind - 1][1] + dp[ind + 1][j][1]);
      }
    }
  }
