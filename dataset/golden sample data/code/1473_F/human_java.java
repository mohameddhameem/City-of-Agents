...
public class F {
...

	{
		...
		for(int i = 0;i < n;i++){
			...
			for(int j = 1;j <= v;j++){
				...
				if(last[j] != -1 && v % j == 0){
					...
					es.add(new Edge(i, last[j], 300000001));
		...
		out.println(base - maximumFlowDinicNoRec(compileWD(sink+1, es), src, sink));
